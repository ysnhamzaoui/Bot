...
namespace OuiBot.Dialogs
{
    ...
    public class HotelsDialog : IDialog<object>
    {
        ...
        private IForm<HotelsQuery> BuildHotelsForm()
        {
            ...
            {
                await context.PostAsync($"Ok. On cherche des hôtels à {state.Ville} de {state.Date.ToString("MM/dd")} to {state.Date.AddDays(state.Nuits).ToString("MM/dd")}...");
            };

            return new FormBuilder<HotelsQuery>()
                .Field(nameof(HotelsQuery.Ville))
                .Message("On recherche des hôtels sur {Ville}...")
                .AddRemainingFields()
        ...
        private async Task ResumeAfterHotelsFormDialog(IDialogContext context, IAwaitable<HotelsQuery> result)
        {
            try
            {
                ...
                await context.PostAsync($"On a trouvé en total {hotels.Count()} hotels pour vos dates:");
            ...
            catch (FormCanceledException ex)
            {
                ...
                {
                    reply = "Vous avez annulé l'opertaion.";
                }
                else
                {
                    reply = $"Oops! Probleme technique: {ex.InnerException.Message}";
                }
        ...
        private async Task<IEnumerable<Hotel>> GetHotelsAsync(HotelsQuery searchQuery)
        {
            ...
            for (int i = 1; i <= 5; i++)
            {
                ...
                {
                    Name = $"{searchQuery.Ville} Hotel {i}",
                    Location = searchQuery.Ville,
                    Rating = searchQuery.Classement,
